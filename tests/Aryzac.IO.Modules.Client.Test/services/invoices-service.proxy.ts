...
export class InvoicesService {
  public async changeDueDateInvoiceCommand(
    id: string,
    command: ChangeDueDateInvoiceCommand
  ...
  public async createInvoiceCommand(
  ...
  public async getInvoiceByIdQuery(
    id: string
  ): Promise<AsyncData<InvoiceDto | null, any>> {
    const config = useRuntimeConfig();
    let url = `${config.public.invoicesServiceApiBaseUri}/api/v1/invoice/${id}`;
  ...
  public async getInvoicesQuery(): Promise<
    AsyncData<InvoiceDto[] | null, any>
  > {
    const config = useRuntimeConfig();
    let url = `${config.public.invoicesServiceApiBaseUri}/api/v1/invoice`;
    return await useLazyFetch<InvoiceDto[]>(url, {
  ...
  public async getInvoicesByClientIdQuery(
    clientId: string
  ): Promise<AsyncData<InvoiceDto[] | null, any>> {
    const config = useRuntimeConfig();
